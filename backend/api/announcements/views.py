...
def create(request):
    Announcement(client_id=request.data['client_id'], title=request.data['title'], description=request.data['description']).save()
    return response_success(message='Announcement created successfully.')

@api_view(["GET"])
@permission_classes([IsAuthenticated])
def read(request):
    announcement = Announcement.objects.all().values('id', 'client_id', 'title', 'description', 'created_at')
